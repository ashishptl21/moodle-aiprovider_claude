...
class action_form extends action_settings_form {
    ...
    protected function definition(): void {
        ...
        $this->providername = $this->_customdata['providername'] ?? 'aiprovider_claude';
    ...
    public function validation($data, $files): array {
        ...
        if (!empty($data['modelextraparams'])) {
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                $errors['modelextraparams'] = get_string('invalidjson', 'aiprovider_claude');
            }
        }
    ...
    protected function add_model_fields(int $modeltype): void {
        global $PAGE;
        $PAGE->requires->js_call_amd('aiprovider_claude/modelchooser', 'init');
        $mform = $this->_form;
        ...
        if (
            ...
            $defaultmodel = key(helper::get_models());
        } else {
        ...
        $mform->addElement(
            'select',
            'modeltemplate',
            get_string("action:{$this->actionname}:model", 'aiprovider_claude'),
        ...
        $mform->addHelpButton('modeltemplate', "action:{$this->actionname}:model", 'aiprovider_claude');

        $mform->addElement('hidden', 'model', $defaultmodel);
        $mform->setType('model', PARAM_TEXT);

        $mform->addElement('text', 'custommodel', get_string('custom_model_name', 'aiprovider_claude'));
    ...
    protected function get_model_list(int $modeltype): array {
        ...
        foreach (helper::get_models() as $model) {
            if (in_array($modeltype, (array) $model->model_type())) {
